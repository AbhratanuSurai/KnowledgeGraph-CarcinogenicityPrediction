...
# class ExpandableCondition(object):
#   def __init__(self, nested_query_sql, query, conditions, table, argss, extra_conditions):
#     self.sql              = nested_query_sql
#     self.query            = query
...
#       query.create_conditions(self.conditions, self.table, *args)
#     self.conditions.append(extra_conditions)
class ExpandableBinding(object):
  def __init__(self, query, conditions, table):
    self.query               = query
    self.conditions          = conditions
    self.table               = table
    self.expanded_conditions = []
    self.expanded_bindings   = {}
    
  def add_binding(self, var, binding, expanded_binding):
    var.bindings.insert(0, binding)
    self.expanded_bindings[var] = expanded_binding
    
  def __str__(self): return self.sql
  
  def expand(self):
    all_conditions = [self.query.conditions]
    for table in self.query.name_2_table.values(): all_conditions.append(table.join_condition)
    
    for var in self.expanded_bindings:
      for conditions in all_conditions:
        for i, condition in enumerate(conditions):
          conditions[i] = conditions[i].replace(var.bindings[0], "=%s" % self.expanded_bindings[var])
      var.bindings.remove(self)
      var.bindings.append(self.expanded_bindings[var])
      
      
    self.query.tables.append(self.table)
    self.conditions.extend(self.expanded_conditions)
...
class SQLQuery(FuncSupport):
  def __init__(self, name):
    ...
    self.optional                 = False
    self.has_one                  = False
    self.vars_used_outside_tables = set()
  ...
  def parse_triples(self, triples):
    ...
    for triple in self.triples: # Pass 4: compute number of bindings per variable
      if   isinstance(triple, (Bind, Filter)):
        for var_name in triple.var_names:
          self.vars_used_outside_tables.add(self.parse_var(var_name))
      if   isinstance(triple, (Bind, Filter, Block)): continue
      s, p, o = triple
    ...
    print("XXX", { var : var.nb_table for var in self.vars.values() })
    print("YYY", self.vars_used_outside_tables)
    ...
    for triple in self.triples: # Pass 5: pre-register bindings for preliminary table
      ...
      # print("-", triple, triple.local_table_type.startswith("prelim"), triple.consider_p)
      # if triple.local_table_type.startswith("prelim") and not triple.consider_p:
      #   if (s.name == "VAR") and self.parse_var(s).nb_table == 0: continue
      #   if (o.name == "VAR") and self.parse_var(o).nb_table == 0: continue
      #   triple.to_skip = True
      #   extra = ""
    ...
    for triple in self.triples: # Pass 6: create triples tables and conditions
      ...
      if triple.local_table_type.startswith("prelim") and not triple.consider_p:
        ...
        expandable_binding = ExpandableBinding(self, conditions, table)
        if s.name == "VAR":
          s = self.parse_var(s)
          expandable_binding.add_binding(s, """IN (SELECT s FROM %s%s)""" % (triple.local_table_type, extra), "%s.s" % table.name)
        if o.name == "VAR":
          o = self.parse_var(o)
          expandable_binding.add_binding(o, """IN (SELECT o FROM %s%s)""" % (triple.local_table_type, extra), "%s.o" % table.name)
        conditions = expandable_binding.expanded_conditions
      else:
        self.tables.append(table)
